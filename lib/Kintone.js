...
module.exports = class Kintone {
  static async uploadCsv(config, input) {
    ...
    const convertedNewRecords = Kintone.convertNewRecords(newRecords);
  ...
  static async checkExistRecords(config, records) {
    let newRecords = [];
    let updateRecords = [];
  ...
  static convertNewRecords(newRecords) {
  ...
  static convertUpdateRecords(newRecords, updateKey) {
    return _.map(newRecords, (record) => {
      let converted = {};
      converted.updateKey = {
        field: updateKey,
        value: record[updateKey],
      };
      converted.record = {};
      _.forEach(record, (value, key) => {
        if (updateKey === key) {
